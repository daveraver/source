...
def acestreams(id=None, url=None, infohash=None, title="", iconimage="", plot=""):
    ...
    if not server.available:
	    ...
	    logger("Arquitectura detectada: %s" % arch) 

	    if arch in ['aarch64', 'arm64-v8a', 'armv8l']:
		AndroidActivity = None 
		if id: 
		    AndroidActivity = 'StartAndroidActivity("", "org.acestream.action.start_content", "", "acestream:?content_id=%s")' % id 	  	 				    xbmc.executebuiltin(AndroidActivity) 
		    return 
		else: import glob 
		...
		if AndroidActivity: 
		   if xbmcgui.Dialog().yesno(HEADING, translate(30041)): 
		    ...
		    xbmcgui.Dialog().ok(HEADING, "No se encontr  la app AceStream instalada") 
		    return 

	    else: 
		possible_acestream_bin = "/data/data/org.acestream.engine/files/acestreamengine" 
		if os.path.exists(possible_acestream_bin): 
		    acestream_executable = possible_acestream_bin 
		    cmd_stop_acestream = ["pkill", "acestreamengine"] 
		    logger("Usando binario AceStream desde %s" % acestream_executable) 
		else: 
		    xbmcgui.Dialog().ok(HEADING, "No se encontr  el binario AceStream en esta arquitectura Android") 
		    return


        else:
              # Otras arquitecturas Android (ej. x86 en emuladores)
              possible_acestream_bin = "/data/data/org.acestream.engine/files/acestreamengine"
              if os.path.exists(possible_acestream_bin):
                    acestream_executable = possible_acestream_bin
                    cmd_stop_acestream = ["pkill", "acestreamengine"]
